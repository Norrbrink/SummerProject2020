...
class Electron:
    ...
    def traj(self):
        ...
        for instant in range(0,period):
            x += 1/2*v*dt   
            #Find vector which bisects two steps in velocity 
            t = 1/2 * self.dipole(x) * self.q * dt / self.m
            #Rescale to meet requirement that speed remains constant with time
            s = 2 * t / (1+sp.dot(t,t))
            
            #Determine velocity 
            v_minus = v +  1/2 * self.q * self.E * dt / (self.m * vAc) 
            v_prime = v_minus + sp.cross(v_minus,t)
            v_plus = v_minus + sp.cross(v_prime,s)
            v = v_plus + 1/2 * self.q * self.E  * dt / (self.m * vAc)
            
            #Determine position at that instand in form [x,y,z]
            x += 1/2* v * dt
            
            #Append current position and velcity to list of previous ones
            R[instant] = sp.array(x)
            V[instant] = sp.array(v)
            
            #Dynamically set timestep    
